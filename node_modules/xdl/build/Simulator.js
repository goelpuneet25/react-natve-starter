...
const profileMethod_1 = require("./utils/profileMethod");
let _lastUrl = null;
...
async function ensureXcodeInstalledAsync() {
    const promptToOpenAppStoreAsync = async (message) => {
        ...
        if (confirm) {
            ...
            internal_1.Xcode.openAppStore(internal_1.Xcode.appStoreId);
        }
    };
    const version = profileMethod_1.profileMethod(internal_1.Xcode.getXcodeVersion)();
    if (!version) {
...
async function isSimulatorInstalledAsync() {
    ...
    const _isSimulatorInstalledAsync = async () => {
        ...
        if (!result) {
            // This error may occur in CI where the users intends to install just the simulators but no Xcode.
            internal_1.Logger.global.error("Can't determine id of Simulator app; the Simulator is most likely not installed on this machine. Run `sudo xcode-select -s /Applications/Xcode.app`");
            return false;
        }
        if (result !== 'com.apple.iphonesimulator' &&
            result !== 'com.apple.CoreSimulator.SimulatorTrampoline') {
            // TODO: FYI
            internal_1.Logger.global.warn("Simulator is installed but is identified as '" + result + "'; don't know what that is.");
            return false;
        }
        ...
        try {
            ...
            if (e.isXDLError) {
                ...
                internal_1.Logger.global.warn(`Unable to run simctl: ${e.toString()}`);
...
async function ensureSimulatorOpenAsync({ udid } = {}, tryAgain = true) {
    ...
    if (!udid) {
        ...
        if (simulatorOpenedByApp === null || simulatorOpenedByApp === void 0 ? void 0 : simulatorOpenedByApp.udid) {
            ...
            udid = (_a = _getDefaultSimulatorDeviceUDID()) !== null && _a !== void 0 ? _a : (await getFirstAvailableDeviceAsync()).udid;
        }
    }
...
function _getDefaultSimulatorDeviceUDID() {
    try {
        const defaultDeviceUDID = child_process_1.execSync(`defaults read com.apple.iphonesimulator CurrentDeviceUDID`, { stdio: 'pipe' }).toString();
...
async function waitForSimulatorAppToStart() {
    return waitForActionAsync({
        interval: 50,
        action: internal_1.SimControl.isSimulatorAppRunningAsync,
    });
}
...
async function _downloadSimulatorAppAsync(url, downloadProgressCallback) {
    ...
    try {
        await internal_1.downloadAppAsync(url, dir, { extract: true }, downloadProgressCallback);
    }
    catch (e) {
        fs_extra_1.default.removeSync(dir);
        throw e;
...
async function openUrlInSimulatorSafeAsync({ url, udid, isDetached = false, sdkVersion, devClient = false, projectRoot, exp = config_1.getConfig(projectRoot, { skipSDKVersionRequirement: true }).exp, skipNativeLogs = false, }) {
    let simulator = null;
    try {
        simulator = await profileMethod_1.profileMethod(ensureSimulatorOpenAsync)({ udid });
    }
    catch (error) {
        return {
            success: false,
    ...
    try {
        if (devClient) {
            bundleIdentifier = await profileMethod_1.profileMethod(internal_1.BundleIdentifier.configureBundleIdentifierAsync)(projectRoot, exp);
            await profileMethod_1.profileMethod(assertDevClientInstalledAsync)(simulator, bundleIdentifier);
            if (!skipNativeLogs) {
        ...
        await profileMethod_1.profileMethod(internal_1.SimControl.openURLAsync, 'SimControl.openURLAsync')({ url, udid: simulator.udid });
    }
    catch (e) {
        if (e.status === 194) {
...
async function openProjectAsync({ projectRoot, shouldPrompt, devClient, udid, scheme, skipNativeLogs, }) {
    if (!(await profileMethod_1.profileMethod(isSimulatorInstalledAsync)())) {
        return {
            success: false,
    ...
    const result = await profileMethod_1.profileMethod(openUrlInSimulatorSafeAsync)({
        udid: device === null || device === void 0 ? void 0 : device.udid,
        url: projectUrl,
...
async function streamLogsAsync({ bundleIdentifier, udid, }) {
    if (internal_1.SimControlLogs.isStreamingLogs(udid)) {
        return;
    }
...
async function sortDefaultDeviceToBeginningAsync(devices) {
    var _a;
    const defaultUdid = (_a = _getDefaultSimulatorDeviceUDID()) !== null && _a !== void 0 ? _a : (await getFirstAvailableDeviceAsync()).udid;
    if (defaultUdid) {
        let iterations = 0;
