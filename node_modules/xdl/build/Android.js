...
async function startEmulatorAsync(device) {
    internal_1.Logger.global.info(`\u203A Opening emulator ${chalk_1.default.bold(device.name)}`);
...
async function openUrlAsync({ url, device, isDetached = false, sdkVersion, devClient = false, exp, projectRoot, }) {
    ...
    try {
        ...
        internal_1.Logger.global.info(`\u203A Opening ${chalk_1.default.underline(url)} on ${chalk_1.default.bold(bootedDevice.name)}`);
...
async function openProjectAsync({ projectRoot, shouldPrompt, devClient = false, device, scheme, }) {
    try {
        await startAdbReverseAsync(projectRoot);
        const projectUrl = await internal_1.UrlUtils.constructDeepLinkAsync(projectRoot, { scheme });
