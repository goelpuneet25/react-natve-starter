...
const helpGroupOrder = ['auth', 'core', 'client', 'info', 'publish', 'build', 'credentials', 'eas', 'notifications', 'url', 'webhooks', 'upload', 'eject', 'experimental', 'internal'];
...
_commander().Command.prototype.asyncActionProjectDir = function (asyncFn, options = {}) {
  ...
  return this.asyncAction(async (projectRoot, ...args) => {
    ...
    const logStackTrace = (chunk, logFn, nestedLogFn) => {
      ...
      for (let i = 0; i <= lastFrameIndexToLog; i++) {
        ...
        if (fileNameOrUrl) {
          ...
          isCollapsed = _metroConfig().INTERNAL_CALLSITES_REGEX.test(fileNameOrUrl); // Unless the user is in debug mode, skip printing the collapsed files.
      ...
      onFinishBuildBundle: (err, startTime, endTime) => {
        ...
        if (bar) {
          ...
          if (err) {
            ...
            _log().default.log(_chalk().default.green(`Finished building JavaScript bundle in ${endTime.getTime() - startTime.getTime()}ms.`));
          }
        }
      },
      updateLogs: updater => {
    ...
    if (options.checkConfig && (await _xdl().ProjectSettings.getCurrentStatusAsync(projectRoot)) !== 'running') {
      ...
      const status = await _xdl().Doctor.validateWithoutNetworkAsync(projectRoot, {
